...
def main() -> None:
    p=argparse.ArgumentParser(); p.add_argument("--input",type=Path,action="append",required=True,help="MODEL=path/to/development_predictions.npz; repeat for every model."); p.add_argument("--output-dir",type=Path,required=True); a=p.parse_args(); a.output_dir.mkdir(parents=True,exist_ok=True)
    pieces=[]
    for item in a.input:
