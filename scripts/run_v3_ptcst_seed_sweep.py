...
def main():
    p=argparse.ArgumentParser(); p.add_argument('--data-root',type=Path,default=ROOT/'data/lseg_v3'); p.add_argument('--run-root',type=Path,default=ROOT/'runs/v3_ptcst_seed_sweep'); p.add_argument('--seeds',nargs='+',type=int,default=[7,19,43,71,101]); p.add_argument('--epochs',type=int,default=100); a=p.parse_args(); a.run_root.mkdir(parents=True,exist_ok=True)
    for seed in a.seeds:
        run=a.run_root/f'seed_{seed}'
        if not (run/'metrics.json').exists():
            subprocess.run([sys.executable,str(ROOT/'scripts'/'run_v3_ptcst_method.py'),'--model-type','PTCST','--data-root',str(a.data_root),'--run-dir',str(run),'--epochs',str(a.epochs),'--early-stopping-patience','10','--batch-dates','32','--seed',str(seed)],check=True)
    reports=[]
    for seed in a.seeds:
