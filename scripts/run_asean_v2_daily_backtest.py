...
from src.asean_v2 import cost_aware_mvo_vector_cost, ledoit_covariance_min_history
...
def main() -> None:
    ...
    for country, country_panel in panel.groupby("country",sort=True):
        ...
        for date, day in country_panel.groupby("date",sort=True):
            ...
            if task:
                ...
                rebalance_records.append({"country":country,"signal_date":signal_date,"execution_date":date,"assets":len(rics),"valid_risk_assets":risk.get("valid_assets"),"risk_fallback":risk_fallback,"turnover":turnover,"cost":cost,"solver_status":status})
    ...
    summary=daily.groupby("country").agg(days=("date","count"),mean_daily_net_return=("net_return","mean"),daily_volatility=("net_return","std"),mean_turnover_rebalance=("turnover","mean"),missing_valuations=("missing_valuations","sum")).reset_index(); summary["annualized_sharpe"]=summary.mean_daily_net_return/summary.daily_volatility*np.sqrt(252); summary.to_csv(a.run_dir/"summary.csv",index=False)
