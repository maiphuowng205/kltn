...
def main():
    p=argparse.ArgumentParser(); p.add_argument('--data-root',type=Path,default=ROOT/'data/lseg_v3'); p.add_argument('--forecast-run',type=Path,required=True); p.add_argument('--run-dir',type=Path,default=ROOT/'runs/v3_ptcst_ablations'); a=p.parse_args(); a.run_dir.mkdir(parents=True,exist_ok=True)
    ...
    for strategy in strategies:
        ...
        for i,dv in enumerate(dates):
            ...
            else: target,solver=cost_aware_mvo(mu,cov,w_pre,valid,cost=0.001 if strategy.endswith('CA-MVO') else 0.0,turnover_fixed=float(exited_weight))
    ...
    report={'created_at_utc':datetime.now(timezone.utc).isoformat(),'strategies':strategies,'forecast_run':str(a.forecast_run),'test_dates':len(dates)}; (a.run_dir/'metrics.json').write_text(json.dumps(report,indent=2),encoding='utf-8'); print(summary.to_string(index=False)); print(json.dumps(report,indent=2))
