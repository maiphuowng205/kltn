...
def main() -> None:
    ...
    for country in ("Indonesia", "Malaysia", "Philippines", "Singapore", "Thailand"):
        ...
        (output / "curated" / "daily_panel_v2" / key).mkdir(exist_ok=True)
        (output / "curated" / "universe_weekly_v2" / key).mkdir(exist_ok=True)
        (output / "model_ready" / "weekly_features_targets_v2" / key).mkdir(exist_ok=True)
        panel.to_parquet(output / "curated" / "daily_panel_v2" / key / "part.parquet", index=False)
        universe[columns].to_parquet(output / "curated" / "universe_weekly_v2" / key / "part.parquet", index=False)
        universe.to_parquet(output / "model_ready" / "weekly_features_targets_v2" / key / "part.parquet", index=False)
