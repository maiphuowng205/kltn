...
def main():
    ...
    manifest={'created_at_utc':datetime.now(timezone.utc).isoformat(),'git_commit':git_commit,'freeze_id':freeze.get('freeze_id'),'sources':{'baseline':portable_path(a.baseline_run),'portfolio':portable_path(a.portfolio_run),'method':portable_path(a.method_run),'ablation':portable_path(a.ablation_run)},'tables':['forecast_summary.csv','portfolio_summary.csv','ptcst_ablation_summary.csv','ptcst_method_summary.csv']}; (a.run_dir/'run_manifest.json').write_text(json.dumps(manifest,indent=2),encoding='utf-8'); print(json.dumps(manifest,indent=2))
